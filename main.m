...
% Width & Length of Original Image
s0 = L - 0.25 * W;
...
for i2 = ys
    for i1 = xs
        ...
        BW = zeros(size(SW, 1), size(SW, 2));
        
        for row = 1 : size(SW, 1)
            for col = 1 : size(SW, 2)
                
                Landa = omega + k;  % multi-valued adaptive threshold for convert to binary image
                if SW(row, col) < Landa
                    BW(row, col) = 1;
                else
                    BW(row, col) = 0;
...
%% Iris outer contour Detection - Pupil
